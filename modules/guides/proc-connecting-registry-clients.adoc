...
+
You’ll use the service account information that you saved to configure your applications to connect to your {registry} instances later when you're ready. For example, if you plan to use https://github.com/edenhill/kafkacat[Kafkacat^] to interact with your Kafka instance and deserialize Avro messages using {registry}, you'll use this information to set your {registry} URL in the client environment variables.
+
IMPORTANT: For details on granting access to service accounts and user accounts, see link:{base-url}{access-mgmt-service-registry-url}[Managing account access in Red Hat OpenShift Service Registry^].
