...
[role="_abstract"]
For this quick start, the Kafka topic that the Quarkus example application references is called `prices`. You need to create this topic in {product-long-kafka} so that the Quarkus application can interact with it.
...
image::kafka-quarkus-kafka/sak-create-topic.png[Image of wizard to create a topic]
+
...
.Verification
ifdef::qs[]
* Does the `prices` topic appear on the *Topics* page?
endif::[]
ifndef::qs[]
* Verify that the `prices` topic appears on the *Topics* page.
endif::[]
