...
.Prerequisites
ifndef::qs[]
* You have a service account with access to Kafka and {registry} instances and have stored your credentials securely (see link:{base-url}{getting-started-url}[Getting started with Kafka^] and link:{base-url}{getting-started-service-registry-url}[Getting started with {registry}^]).
